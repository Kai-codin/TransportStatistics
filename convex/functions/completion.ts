...
async function lookupOperatorByCode(ctx: QueryCtx, code: string) {
...
export const getOperatorByAnyCode = query({
  ...
  handler: async (ctx, args) => {
    return lookupOperatorByCode(ctx, args.code);
  },
});
...
export const getOperatorCompletionStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    const operator = await lookupOperatorByCode(ctx, args.operator_code);
    if (!operator) {
      return null;
    }
